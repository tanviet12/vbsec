...
1. Nạp phần phát hiện của cả bộ rule bằng **1 lệnh** `bash <skill-dir>/references/load-rules.sh <lang...>` (overlay đã thay generic) → hiểu intent, severity, search patterns gợi ý. Phần Examples/Fix recommendation chỉ Read khi rule có finding CRITICAL/HIGH (chi tiết trong workflow)
